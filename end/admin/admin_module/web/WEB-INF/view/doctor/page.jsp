...
<div><!-- 从request中取出传入的参数，放入本页变量中 -->
  ...
  <p class="green-black ">
    ...
    <c:if test="${pageNumber>1}">
      <a href="/doctor/dopage?size=${pageNumber-1}&id=${Id}">上一页 </a>
    </c:if>
    当前第
    &nbsp;&nbsp;
    ${pageNumber}
    ...
    <c:if test="${pageNumber!=totalPage}">
      <a href="/doctor/dopage?size=${pageNumber+1}&id=${Id}">下一页</a>
    </c:if>
    &nbsp;&nbsp;
    <span>选择:</span>
    <select name="pageSelect" onchange="window.location.href=this.options[selectedIndex].value">
      <c:forEach var="i" begin="1" end="${totalPage}" step="1">
        <c:if test="${i==pageNumber}">
          <option value="/doctor/dopage?size=${i}&id=${Id}" selected>${i}</option>
        </c:if>
        <c:if test="${i!=pageNumber}">
          <option value="/doctor/dopage?size=${i}&id=${Id}">${i}</option>
        </c:if>
      </c:forEach>
    </select>
  </p>
